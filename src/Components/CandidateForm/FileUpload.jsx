...
function FileUpload({ label, onFileChange, acceptedFiles, setAcceptedFiles }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (!selectedFile) {
      ...
      if (label === "Photo  (Allowed Formats: JPG or PNG)") {
    ...
    <Form.Group className="mb-3">
      ...
      {!file ? (
        <div>
          ...
          <input
            type="file"
            accept={label === "Photo  (Allowed Formats: JPG or PNG)" ? ".jpg, .jpeg, .png" : ".pdf"}
            ref={fileInputRef}
            style={{ display: "none" }}
          ...
          {progress === 100 && (
            ...
            >
              <div className="pdf-image" style={{ height: "50px" }}>
                <Image src={label === "Photo  (Allowed Formats: JPG or PNG)" ? png : pdf} alt="PDF" width="40px" height="50px" />
              </div>
